...
import { documents,users,vehicles,videos } from '../src/db/schema'
...
if(!existingVideos.length)await db.insert(videos).values({vehicleId:vehicle.id,submittedBy:user.id,youtubeUrl:'https://www.youtube.com/results?search_query=renault+symbol+troca+de+oleo',youtubeId:null,title:'Demonstrações de troca de óleo (busca no YouTube)',component:'Manutenção — motor K4M',approved:true})
