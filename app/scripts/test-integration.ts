...
import { docChunks,documents,tips,users,vehicles } from '../src/db/schema'
...
const answer=await answerQuestion(user.id,'pneus',vehicle.id);assert(answer.sources.length>0);assert(answer.sources.some((x:any)=>Number(x.page)>=1))
